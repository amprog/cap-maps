jQuery(document).ready(function( $ ) {
    ...
    $.getJSON( "/wp-content/plugins/svg_map/data/2015/11057.json")
        .done(function( json ) {
            contact_data = json;
        })
    ...
    if(mobile.mobile==1 || mobile.width<1200) {
        ...
        function drawRegionsMap() {
            ...
            $.getJSON("/wp-content/plugins/svg_map/data/2015/international-reps.json")
                .done(function (json) {
    ...
    function handle(id) {
        ...
        $('.hide_on_mobile').hide();
        c.html('<ul class="contact_list">' +
            '<h3>'+contact.repcountry+'</h3>' +
            ...
            '<li>Contact in PA:  <i class="icon-mail"></i><a href="mailto:'+contact.email+'">'+contact.name2+'</a></li>' +
            '</ul>' +
            '<h3>Countries Represented </h3><ul class="contact_list">' +
            '<li>'+contact.territory+'</li></ul>'
        );
    }
